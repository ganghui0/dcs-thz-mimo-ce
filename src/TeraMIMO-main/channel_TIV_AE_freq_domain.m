function H = channel_TIV_AE_freq_domain(p, K_abs)
% =========================================================================
                ...
                for nt = 1:p.Nt
                    ...
                    if strcmp(p.channelType,'LoS') || strcmp(p.channelType,'Multipath+LoS')
                        ...
                        if  strcmp(p.WaveModelAE,'Plane')
                            ...
                            H_LoS_tmp = Gr_LOS*Gt_LOS*AlphaLoS*a_sv_r_LoS(:)*(a_sv_t_LoS(:)).';
                        else
                    ...
                    if strcmp(p.channelType,'Multipath') || strcmp(p.channelType,'Multipath+LoS')
                        ...
                        for indx_clusters = 1: p.nClustersVec(indx_subc)
                            for indx_rays = 1: p.nRaysVec(indx_subc)
                                ...
                                H_NLoS_c_l(:,:,indx_clusters,indx_rays) = Gr_NLOS(indx_clusters,indx_rays).*Gt_NLOS(indx_clusters,indx_rays).*AlphaNLoS(indx_clusters,indx_rays)*a_sv_r_NLoS(:)*(a_sv_t_NLoS(:)).';
                            end
                        end
                        H_NLoS_tmp = sum(sum(H_NLoS_c_l, 3), 4);
