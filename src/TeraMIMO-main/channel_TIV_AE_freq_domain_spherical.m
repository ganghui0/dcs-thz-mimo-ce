function H = channel_TIV_AE_freq_domain_spherical(p, K_abs)
% =========================================================================
...
H_LoS = zeros(p.Qr*p.Qar, p.Qt*p.Qat, num_subcarries, num_freqs_per_subcarr);
% H_NLoS = zeros(p.Qr, p.Qt, num_subcarries, num_freqs_per_subcarr);
H_NLoS = zeros(p.Qr*p.Qar, p.Qt*p.Qat, num_subcarries, num_freqs_per_subcarr);
        ...
        for nr = 1:p.Nr
            for mt = 1:p.Mt
                ...
                if strcmp(p.channelType,'LoS') || strcmp(p.channelType,'Multipath+LoS')
                    % Phase_Delay_LOS = exp(1j*2*pi*indx_subc/num_subcarries*p.BW*Tau_LoS);
                    ...
                    rowIndices = ((mr-1)*p.Nr+nr-1)*p.Qar + 1 : ((mr-1)*p.Nr+nr)*p.Qar;
                    colIndices = ((mt-1)*p.Nt+nt-1)*p.Qat + 1 : ((mt-1)*p.Nt+nt)*p.Qat;
                    % H_LoS((mr-1)*p.Nr+nr,(mt-1)*p.Nt+nt,indx_subc,:) = H_LoS_tmp;
                    H_LoS(rowIndices, colIndices,indx_subc,:)= H_LoS_tmp;
                end
                    
                if strcmp(p.channelType,'Multipath') || strcmp(p.channelType,'Multipath+LoS')
                    AlphaNLoS = get_NLoS(p, indx_subc, AlphaLoSAmp, ToAClusters, ToARays, ToAMPtot);

                    % Phase_Delay_NLOS = exp(1j*2*pi*indx_subc/num_subcarries*p.BW*ToAMPtot);
                    ...
                    H_NLoS_c_l = zeros(p.Qar,p.Qat,p.nClustersVec(indx_subc),p.nRaysVec(indx_subc));
                    for indx_clusters = 1: p.nClustersVec(indx_subc)
                        for indx_rays = 1: p.nRaysVec(indx_subc)
                            ...
                            alphaNLoS = AlphaNLoS((indx_clusters-1)*size(AlphaLoS,1)+1:indx_clusters*size(AlphaLoS,1),(indx_rays-1)*size(AlphaLoS,2)+1:indx_rays*size(AlphaLoS,2));
                            H_NLoS_c_l(:,:,indx_clusters,indx_rays) = Gr_NLOS(indx_clusters,indx_rays).*Gt_NLOS(indx_clusters,indx_rays).*alphaNLoS.*(a_sv_r_NLoS(:)*(a_sv_t_NLoS(:)).')/sqrt(p.Qt*p.Qr);
                        end
                    end
                    H_NLoS_tmp = sum(sum(H_NLoS_c_l, 4), 3);
                    ...
                    rowIndices = ((mr-1)*p.Nr+nr-1)*p.Qar + 1 : ((mr-1)*p.Nr+nr)*p.Qar;
                    colIndices = ((mt-1)*p.Nt+nt-1)*p.Qat + 1 : ((mt-1)*p.Nt+nt)*p.Qat;
                    H_NLoS(rowIndices, colIndices,indx_subc,:)= H_NLoS_tmp;
                end
