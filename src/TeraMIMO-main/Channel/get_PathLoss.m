        ...
        for aenr = 1:p.Nar
            for aemt = 1:p.Mat
                for aent = 1:p.Nat
                    ...
                    d_SAAEs((aemr-1)*p.Nar+aenr,(aemt-1)*p.Nat+aent) = norm((positionGlobalAERx-positionGlobalAETx));
                end
            end
        end
    end
...
elseif strcmp(p.WaveModelAE,'Sphere')
    ...
    for idx_freq = 1:num_freqs_per_subcarr   
        ...
        % d_TxRx = norm(p.positionRx-p.positionTx);
        % AlphaLoSPhase(:, :, idx_freq) = exp(-1j*2*pi*p.freq(idx_freq,indx_subc).*Tau_LoS).*exp(-1j*2*pi*p.freq(idx_freq,indx_subc).*d_TxRx/p.c);
